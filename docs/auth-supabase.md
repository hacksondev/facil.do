...
- Middleware con `createMiddlewareClient` protege `/panel/*` (excepto rutas de onboarding/login).
...
Endpoints front:
- Login: `/panel/login`
- SignUp: `/panel/onboarding/create-account` (crea user + session)
...
- RLS: agrega filtros por `auth.uid()` donde aplique y roles específicos para admin/panel (usando service role).***
